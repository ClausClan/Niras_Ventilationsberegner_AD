...
// Adskiller visuel tegneretning fra fysisk flowretning og spytter ægte fuldt udregnede fittings ud!
function createTransitionComponent(visualInlet, visualOutlet, airflow, globalFlowType, previousComponent) {
    ...
    // 1. Identificer VISUEL overgangstype (Hvad der er tegnet på skærmen - oppefra og ned i træet)
    ...
    // 3. FYSISKE Beregninger (Korrigeret for Udsugning/Indblæsning internt)
    ...
    // Navngivning til UI (Beskriver tegneretning for at forhindre forvirring)
    ...
    // Nedarv termodynamik for at lette beregningen for genberegningsmotoren
    const pProps = previousComponent && previousComponent.properties ? previousComponent.properties : {};

    // 4. Returner en ægte, fuldt udregnet komponent! 
    return {
        id: 'transition_' + Date.now() + Math.floor(Math.random() * 1000),
        type: visualFittingType,     // CRITICAL: Must be 'expansion' etc. so Diagram.js draws a cone!
        name: name,
        details: details,
        ...
        properties: {
            type: visualFittingType, // CRITICAL: So recalculateSystem recognizes it if re-run
            angle: angle,
        ...
        // CRITICAL FIX: Generer State DIREKTE, så tabellen har værdierne og de ikke nulstilles!
        state: {
            airflow_in: airflow,
...
function calculateComponentPhysics(component, incomingFlow, incomingTemp, incomingDim, globalParams, calculateThermodynamicsFlag = true) {
    ...
    if (component.type === 'straightDuct') {
        ...
        if (p.type === 'bend_circ') {
            ...
            // Hvis brugeren har indsat en manuel overgang, udregnes den her. 
            // Auto-overgange ankommer allerede fuldt udregnede.
            ...
            newCalc = {
                ...
                portPressureLoss: { 'outlet_straight': loss_b1, 'outlet_branch': loss_b2 }, 
...
function recalculateSystem() {
    ...
    // Vi sorterer KUN auto-overgange fra midlertidigt for at kunne fange om de er nødvendige stadig
    ...
    // Reset grafen så den kan genopbygges med opdaterede dimensioner og evt. nye overgange
    ...
    function traverseAndCalculate(nodeId, incomingFlow, incomingTemp, incomingDim, parentId, parentPort) {
        ...
        // --- RETTELSE: Vi fjerner dobbelt-vendingen. Send dem ind præcis som de optræder i træet (fra rod mod blad)! ---
        if (incomingDim && childAttachDim && !physics.areDimensionsEqual(incomingDim, childAttachDim)) {
            ...
            if (transition) {
                // Transition har allerede fået bygget sin 'state' indeni createTransitionComponent.
                // Vi sender den direkte i stateManager!
    ...
    function traverseAndCalculateThermodynamics(nodeId, incomingTemp) {
        ...
        // Bemærk: Denne bruger NU grafens egne kanter (inklusiv de nyskabte overgange!),
        // så overgange udregnes også for temperaturtab!
        ...
        const runThermo = (t_in) => {
            ...
            // Overstyr ikke den fulde state, kun opdater termodynamik felterne!
...
window.handleInlineComponentSubmit = function (event) {
    ...
    if (component) {
        ...
        // === IDENTIFICER AUTO-OVERGANG FØR INDSÆTTELSE ===
        if (actualParentId && parentComp) {
            ...
            // Indsæt Transitionen I GRAFEN før the ny komponent tilknyttes
