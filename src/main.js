...
// Adskiller visuel tegneretning fra fysisk flowretning og spytter ægte fittings ud!
function createTransitionComponent(visualInlet, visualOutlet, airflow, globalFlowType, previousComponent) {
    ...
    // 4. Returner en rigtig 'fitting' komponent til både diagram og physics
    return {
        id: 'transition_' + Date.now() + Math.floor(Math.random() * 1000),
        type: 'fitting',             // CRITICAL: Gør at den fanges af 3D diagrammet!
        fittingType: visualFittingType, // CRITICAL: Gør at diagrammet ved, hvordan den skal tegnes
        name: name,
        details: details,
        ...
        properties: {
            type: visualFittingType, // CRITICAL: Gør at genberegningsmotoren fanger den som rigtig type!
            angle: angle,
            ...
            zeta: zeta,                  // Bevares for transparens
            pressureLoss: pressureLoss   // Bevares for transparens
        },
        state: {} 
    };
}
...
function calculateComponentPhysics(component, incomingFlow, incomingTemp, incomingDim, globalParams, calculateThermodynamicsFlag = true) {
    ...
    if (component.type === 'straightDuct') {
        ...
        if (p.type === 'bend_circ') {
            ...
            // ALT DIMENSIONSÆNDRING (Fanger alle udvidelser og overgange, både auto og manuelle)
...
function recalculateSystem() {
    ...
    function traverseAndCalculate(nodeId, incomingFlow, incomingTemp, incomingDim, parentId, parentPort) {
        ...
        if (incomingDim && childAttachDim && !physics.areDimensionsEqual(incomingDim, childAttachDim)) {
            ...
            const transition = createTransitionComponent(t_inlet, t_outlet, activeFlow, globalParams.globalFlowType, parentComp);
            if (transition) {
    ...
    function traverseAndCalculateThermodynamics(nodeId, incomingTemp) {
        ...
        const isExhaust = globalParams.globalFlowType === 'merging';
        const childrenEdges = userEdges.filter(e => e.from === nodeId); // Use userEdges here since transitions are handled implicitly
    ...
    if (component) {
        ...
        if (actualParentId && parentComp) {
            ...
            if (pDim && cDim && !physics.areDimensionsEqual(pDim, cDim)) {
                // Sørg for at den regnes "Visuelt" ligesom diagrammet ser den
